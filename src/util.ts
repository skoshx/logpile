...
export const isErrorLog = (log: LogEntry) =>
  ['emergency', 'alert', 'critical', 'error'].includes(log.level);
export const isWarningLog = (log: LogEntry) => log.level === 'warning';
export const isVerboseLog = (log: LogEntry) =>
  ['notice', 'informational', 'debug'].includes(log.level);
...
export function tryCatchSync<DataType = unknown, ErrorType = unknown>(
  fn: () => DataType,
): TryCatchResponse<DataType, ErrorType> {
  try {
