 ...
 * 
 * getLogger({
 *  persist: [consolePersist({ depth: 2, level: 'error' }), filePersist({ file: `/etc/logs/app-logs.log` })],
 ...
 */
export function getLogger<LogType = unknown, OptionsType = unknown>(medium: LogMedium<LogType, OptionsType>) {
  ...
  // Logging functions
  const emergency = (...args: TypeOrPrimitives<LogType>[]) => persist('emergency', ...args);
  const emerg = (...args: TypeOrPrimitives<LogType>[]) => persist('emergency', ...args);
  const alert = (...args: TypeOrPrimitives<LogType>[]) => persist('alert', ...args);
  const critical = (...args: TypeOrPrimitives<LogType>[]) => persist('critical', ...args);
  const crit = (...args: TypeOrPrimitives<LogType>[]) => persist('critical', ...args);
  const error = (...args: TypeOrPrimitives<LogType>[]) => persist('error', ...args);
  const err = (...args: TypeOrPrimitives<LogType>[]) => persist('error', ...args);
  const notice = (...args: TypeOrPrimitives<LogType>[]) => persist('notice', ...args);
  const info = (...args: TypeOrPrimitives<LogType>[]) => persist('info', ...args);
  const debug = (...args: TypeOrPrimitives<LogType>[]) => persist('debug', ...args);

  // Searching functions
    
  /*function searchLogs<T = unknown>(
    logs: LogEntry<T>[],
    search: Partial<T>,
  ...
  function searchLogs<T = unknown>(
    ...
    return _searchLogs(logs, search, options);
  }*/
  ...
  return { emergency, emerg, alert, critical, crit, error, err, notice, info, debug, searchLogs };
}
